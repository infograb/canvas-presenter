...
1. 원본 슬라이드의 문구·비율·미디어를 바꾸지 않습니다. 변환이 필요하면 `inline`처럼 **사본**을 만듭니다. 화면 모드도 슬라이드 색을 건드리지 않습니다. `--slide-backdrop`은 두 모드에서 모두 흰색입니다.
...
15. 색은 토큰만 씁니다. 개별 규칙에 색 리터럴을 적지 않습니다. 캔버스 화면의 팔레트는 `:root`의 토큰 한 벌이고, 어두운 모드는 같은 이름을 다시 정의한 것뿐입니다. 슬라이드 쇼의 색은 두 모드 공통이라 예외입니다.
15-1. 매니페스트의 그룹 `color`는 사용자 값입니다. 글자색으로 그대로 쓰지 말고 `--group-ink`·`--group-border`·`--group-fill` 같은 테마별 파생 토큰을 거칩니다. 밝은 배경을 전제로 고른 색이 어두운 모드에서 사라지지 않아야 합니다.
15-2. 미니맵과 배경 점의 색은 React가 prop으로 넘겨야 하므로 `readThemeColors()`가 CSS 토큰을 읽어 전달합니다. 이 값을 `requestAnimationFrame`으로 미루지 않습니다. 백그라운드 탭에서는 rAF가 오지 않아 색이 영영 갱신되지 않습니다.
15-3. `--brand-wash` 위의 글자는 `--brand`가 아니라 `--brand-strong`을 씁니다. 두 모드 모두 `--brand`로는 4.5:1을 넘기지 못합니다.
